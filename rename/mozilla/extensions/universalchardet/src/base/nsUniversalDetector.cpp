  ...
  case eHighbyte:
  {
	  ...
	  for (i = 0; i < NUM_OF_CHARSET_PROBERS; i++)
	  {
		  if (mCharSetProbers[i])
		  {
			  ...
			  {
				  bFound = true;
				  if (mCharSetProbers[i]->GetConfidence() > fMaxConfidence)
				  {
					  fMaxConfidence = mCharSetProbers[i]->GetConfidence();
					  mDetectedCharset = mCharSetProbers[i]->GetCharSetName();
				  }

				  //z mDone = PR_TRUE;
				  //z mDetectedCharset = mCharSetProbers[i]->GetCharSetName();
				  //z return NS_OK;
			  }
		  }
	  }
	  ...
	  {
		  mDone = PR_TRUE;
		  //mDetectedCharset = mCharSetProbers[nIndex]->GetCharSetName();
		  return NS_OK;
	  }
  }
	  break;
